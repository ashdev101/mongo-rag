...
## Claude QnA System (New)

A standalone sequential tool-calling QnA system using **Claude from AWS Bedrock** has been added to test the hypothesis that LLMs should perform sequential tool calls to reach conclusions.
...
### Key Features
- **Sequential Tool Calling**: Claude uses an agentic loop to make multiple tool calls until reaching a conclusion
- **Native Claude API**: Uses AnthropicBedrock client (not LangChain)
- **MongoDB Tools**: List collections, inspect schemas, run aggregation pipelines
- **Conversation History**: Supports follow-up questions with context
- **Strict Domain Guardrails**: Only answers questions about TataPlay HR data

### Documentation
- **Complete Guide**: [docs/CQA_README.md](docs/CQA_README.md)
- **Model Configuration**: [docs/MODEL_INFO.md](docs/MODEL_INFO.md)
- **Testing Scripts**: [scripts/README.md](scripts/README.md)

### Comparison with Existing System

| Feature | Existing (Mongo-RAG) | New (Claude QnA) |
|---------|---------------------|------------------|
| Framework | LangChain + LangGraph | Native Anthropic API |
| Tool Calling | Single-shot with React agent | Sequential agentic loop |
| PII Protection | Pre-query masking | Not implemented yet |
| Model | OpenAI GPT-4 / Bedrock Qwen | Claude Sonnet 4.5 |
| Use Case | Production HR system | Research/testing |
